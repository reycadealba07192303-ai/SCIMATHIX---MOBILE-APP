...
class AdminGradeLevelsScreen extends ConsumerStatefulWidget {
  final String schoolYear;
  
  const AdminGradeLevelsScreen({super.key, required this.schoolYear});

  @override
  ConsumerState<AdminGradeLevelsScreen> createState() => _AdminGradeLevelsScreenState();
}

class _AdminGradeLevelsScreenState extends ConsumerState<AdminGradeLevelsScreen> {
  ...
  Future<void> _fetchLevels() async {
    ...
    try {
      ...
      // Fetch levels filtered by school year
      final levels = await apiService.getLevels(widget.schoolYear);
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(CupertinoIcons.layers, size: 64, color: AppTheme.subtleText.withOpacity(0.5)),
          ...
          Text(
            "Tap the + icon to add a Grade Level.",
  ...
  void _showAddLevelDialog() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AdminAddLevelScreen(schoolYear: widget.schoolYear)),
    ).then((value) {
      if (value == true) _fetchLevels();
    });
  }
