...
class TeacherLeaderboardScreen extends ConsumerStatefulWidget {
  const TeacherLeaderboardScreen({super.key});

  @override
  ConsumerState<TeacherLeaderboardScreen> createState() => _TeacherLeaderboardScreenState();
}

class _TeacherLeaderboardScreenState extends ConsumerState<TeacherLeaderboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchLeaderboards() async {
    ...
    try {
      ...
      String? sectionId;
      if (_scopeIndex == 0 && user?.handledClasses != null) {
        final sectionIds = user!.handledClasses!.map((hc) => hc.sectionId).toSet().toList();
        if (sectionIds.isNotEmpty) {
          sectionId = sectionIds.join(',');
        } else {
          sectionId = 'none'; // So it returns empty
        }
      }
    ...
    return FadeInDown(
      ...
      child: Padding(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              _buildScopeSegment("My Sections", 0),
